...
function SaveJSONProject(url,project,callback) {
    alert(url);
    alert(encodeURI(JSON.stringify(project)));
    $.ajax({
        url: url+'?fileBody='+encodeURI(JSON.stringify(project)),
        cache: false,
        type: "POST",
...
function ShowTaskDubleWindow(task,project,tabName) {
    if (task) {
        if (!taskWindow) {
            taskWindow = new TaskWindow();
        }
